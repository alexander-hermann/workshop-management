		...
		if (!listOfPlannedWorksForWorkingPlatformAfterNow.isEmpty()) {
			...
			if (sizeListAppointmentsAfterNow >= 2) {
				...
				for (listAppointmentCounter = 0; listAppointmentCounter < sizeListAppointmentsAfterNow; listAppointmentCounter++) {
					...
					int differenceBetweenAppointments = (int) ChronoUnit.MINUTES.between(thisAppointmentFromList.getDayWithEndTime(), nextAppointmentFromList.getDayWithStartTime());
				...
				if (timeAfterDuration.isBefore(nextAppointmentForWorkingPlatform.getDayWithStartTime())&& now.getHour() >= 8 && now.getHour() < 15) {
